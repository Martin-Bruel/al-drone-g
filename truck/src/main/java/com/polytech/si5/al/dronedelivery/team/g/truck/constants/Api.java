package com.polytech.si5.al.dronedelivery.team.g.truck.constants;

public class Api {
    public static final int RESTTEMPLATE_TIMEOUT =5000; //m
    public static final int DRONE_TIMEOUT =5000;
    public static final int DRONE_RETRY_CONNECTION=5;
                //////////- Drone APIs-//////////
    public static final String DRONE_API_BASE_URL="drone-api";

              //////////- Warehouse APIs-//////////
    public static final String WAREHOUSE_API_HOST ="localhost";
    public static final String WAREHOUSE_API_PORT ="8086";
    public static final String WAREHOUSE_API_BASE_URL ="warehouse-api";

}
